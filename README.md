...
- Eval Regression Gate Pack under `data/eval_regression_gates/`, applying release-style benchmark gates across scenario evals and trace diagnostics with owner actions, artifact handoffs, and run transparency
...
28m. Run `POST /evals/regression-gate` for release-style benchmark gates across scenario evals and trace diagnostics.
28n. Export `POST /evals/regression-pack` to write the Eval Regression Gate Pack under `data/eval_regression_gates/`.
...
- `POST /evals/regression-gate`: runs the Eval Regression Gate with scenario accuracy thresholds, observability thresholds, unsafe-action checks, zero-external-call checks, review gates, owner actions, artifact handoffs, and run transparency.
- `POST /evals/regression-pack`: writes Markdown and JSON under `data/eval_regression_gates/` with the release decision, benchmark gates, scenario and trace eval handoffs, proof commands, and limitations.
