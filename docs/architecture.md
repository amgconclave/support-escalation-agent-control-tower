...
- **CustomerCommunicationQualityService** scores drafted replies for empathy, specificity, policy compliance, and escalation clarity, then exports role-crew review gates and trace-backed handoff artifacts.
- **SupportOperationsService** builds role crews, delegated task boards, process modes, review gates, run transparency, and artifact handoffs for support leaders, account teams, engineering escalation owners, and operations commanders.
- **SupportOpsSandboxService** simulates local worker execution over delegated support-ops tasks with worker assignment, task budgets, tool transcripts, scale-out decisions, issue-to-handoff loop stages, and dispatch-boundary verification gates.
- **SupportOpsReadinessService** evaluates support-ops crews across deterministic scenarios, checking process-mode coverage, required role delegation, review-gate health, sandbox transcript coverage, and zero external calls.
- **ToolGovernanceService** builds the local tool manifest registry, compares observed tool calls to trusted manifests, scores owner/risk/data/HITL/failure-mode controls, and exports marketplace trust packs.
- **AgentBusCoordinationService** reads local JSONL supervisor bus files when present, summarizes conductor/worker/verifier/repo-radar/UI-bridge handoffs, applies review gates, and exports read-only coordination packs.
...
- Autonomous Support Operations Pack artifacts under ignored `data/support_ops_packs/`
- Support Ops Worker Sandbox Pack artifacts under ignored `data/support_ops_sandbox/`
- Support Ops Crew Readiness Pack artifacts under ignored `data/support_ops_readiness/`
- Tool Governance and Marketplace Trust Pack artifacts under ignored `data/tool_governance_packs/`
- Agent Coordination Bus Pack artifacts under ignored `data/agent_bus_packs/`
...
Runbook Coverage is a governance layer over the local playbook and KB fixtures. It compares active/sample tickets and scenario tickets against `sample_data/playbooks.json` and `sample_data/kb_articles.json`, then writes gap packs under ignored `data/runbook_gap_packs/` without contacting external systems. The audit borrows role-playbook, task-delegation, process-mode, review-gate, artifact-handoff, and run-transparency patterns so missing runbooks become owner-ready remediation work instead of a flat report. The remediation draft export extends that path with human-in-the-loop fixture drafts under `data/runbook_remediation_drafts/`, durable review checkpoints, owner queues, and mutation guards so proposed playbook or KB changes are visible but not automatically applied.
...
The Executive Daily Ops Brief is the daily command-center consumer over the same control outputs. `GET /ops/daily-brief` composes ops analytics, SLO budget, customer health, capacity planning, leadership readiness, and the enterprise risk register into SLA exposure, blocked approvals, engineer load, critical accounts, top risky tickets, control signals, and owner actions. `POST /ops/daily-brief-pack` writes Markdown/JSON under ignored `data/daily_ops_briefs/`. It remains local/mock only and does not call CRM, billing, BI, workforce management, Zendesk, Jira, Slack, Azure, OpenAI, GitHub, or external systems.
...
The Customer Communication Quality service is the pre-dispatch review gate for drafted customer replies. `GET /communications/quality-audit` reads the stored run, draft, KB evidence, QA result, approval ID, tool calls, and SLA state, then applies a deterministic role crew: empathy reviewer, specificity reviewer, policy guardrail reviewer, and engineering escalation reviewer. `POST /communications/quality-pack` writes Markdown/JSON under ignored `data/communication_quality_packs/` with score dimensions, role playbook handoffs, review-gate blockers, run transparency, artifact handoffs, Scenario Dataset coverage, and reviewer actions. It borrows role-crew and review-gate architecture patterns while remaining local/mock only.

The Engineering Escalation Quality service is the internal-dispatch counterpart for Jira-ready handoffs. `GET /escalations/quality-audit` reads the same run state and scores actionability, reproduction evidence, customer impact, routing governance, and noise control before any engineering-facing action can leave the local approval queue. `POST /escalations/quality-pack` writes Markdown/JSON under ignored `data/escalation_quality_packs/` with role-crew findings, trace-backed evidence, human-in-the-loop gate status, and required revisions. It borrows governance, human-in-the-loop, and trace-observability patterns without creating Jira issues, Slack alerts, or external calls.

The Support Operations service is the crew-planning layer for escalation execution. `GET /ops/crew-plan` reads the stored run, ticket, classification, SLA risk, KB evidence, playbook recommendations, drafts, approval state, and trace metadata, then selects a process mode and delegates work across Support Lead, Account Team, Engineering Escalation Owner, and Operations Commander crews. `POST /ops/crew-pack` writes Markdown/JSON under ignored `data/support_ops_packs/` with the delegation board, review gates, run transparency, artifact handoffs, and scenario coverage. It borrows role-crew, task-delegation, process-mode, review-gate, and run-transparency patterns while staying deterministic and local/mock only.

The Support Ops Worker Sandbox is the execution-transparency layer over those delegated tasks. `GET /ops/crew-sandbox` performs dry-run local worker assignment, budget checks, tool transcript generation, scale-out assessment, and verification gates without invoking external workers or providers. `POST /ops/crew-sandbox-pack` writes Markdown/JSON under ignored `data/support_ops_sandbox/` with the worker assignment board and transcript summary. It borrows task-sandbox, run-transparency, worker scale-out, review-gate, and benchmark-discipline patterns while keeping all customer and engineering dispatch blocked behind human approval.

The Support Ops Crew Readiness drill is the eval layer over crew planning and sandbox execution. `GET /ops/crew-readiness-drill` runs deterministic scenario fixtures through the workflow, crew planner, and sandbox, then scores process-mode routing, role coverage, review gates, transcript evidence, and external-call boundaries. `POST /ops/crew-readiness-pack` writes Markdown/JSON under ignored `data/support_ops_readiness/` for reviewer inspection. It borrows role-crew, task-delegation, process-mode, review-gate, task-sandbox, and run-transparency patterns without calling external providers.

The Tool Governance service is the local marketplace and tool-trust layer. `GET /tools/registry` returns manifests for the fake/local support tools with owners, risk tiers, data exposure, allowed actions, human-approval boundaries, failure modes, observed usage, unknown tool references, and marketplace intake policy. `POST /tools/governance-pack` writes Markdown/JSON under ignored `data/tool_governance_packs/` with an approval matrix and production acceptance criteria. It borrows tool-governance and marketplace-intake patterns while keeping all live SaaS, LLM, and external tools disabled by default.

The Agent Bus Coordination service is the local supervisor handoff layer. `GET /ops/agent-bus-audit` reads JSONL inbox/outbox files if a local supervisor bus is present, then reports registered agent roles, message-file health, message routes, required handoffs, review gates, operator queue items, and recent message previews. `POST /ops/agent-bus-pack` writes Markdown/JSON under ignored `data/agent_bus_packs/`. It borrows agent-role, task-delegation, review-gate, artifact-handoff, task-sandbox, and run-transparency patterns while staying read-only: it does not send bus messages, automate the Codex desktop UI, mutate git state, or call GitHub, Azure, OpenAI, Zendesk, Jira, Slack, browser, or network providers.
...
The Postmortem Review service is the closure-governance layer on top of RCA. `GET /incidents/postmortem-review-board` aggregates RCA actions into role-owned lanes, delegated crews, role playbooks, closure gates, signoffs, review cadence, run transparency, and artifact handoffs. `POST /incidents/postmortem-review-pack` writes Markdown/JSON under ignored `data/postmortem_review_packs/` with owner summaries, review-gate status, proof commands, and limitations. It borrows role-crew, task-delegation, review-gate, artifact-handoff, and run-transparency patterns without calling external ticketing, calendar, Slack, Jira, Zendesk, Azure, or OpenAI services.
...
The Escalation Decision Board is the approval gate that composes those local control outputs for a human decision. `GET /escalations/decision-board` reads Finance Impact, Engineering Escalation Quality, Customer Communication Quality, and Support Ops Crew Plan evidence, then returns decision status, role signoffs, review gates, owner actions, artifact handoffs, and run transparency. `POST /escalations/decision-pack` writes Markdown/JSON under ignored `data/escalation_decision_packs/`. It borrows role-crew, task-delegation, process-mode, review-gate, artifact-handoff, run-transparency, and requirements-to-implementation patterns while keeping all customer, Jira, Slack, finance, CRM, Azure, OpenAI, and external actions disabled.
...
The Access Control service is the local authorization governance layer before real identity integration. `GET /security/access-matrix` derives the FastAPI route inventory from OpenAPI and route dependencies, then maps endpoints to least-privilege support, engineering, compliance, incident, reviewer, and platform-admin roles. `POST /security/access-review-pack` writes Markdown/JSON under ignored `data/access_review_packs/` with production scopes, findings, acceptance criteria, and a production authz backlog. It does not change local demo-key enforcement and does not call an IdP, gateway, WAF, Azure, OpenAI, Zendesk, Jira, Slack, GitHub, or external service.

The Enterprise Risk Register service is the consolidated governance layer over existing local controls. `GET /risk/register` reads deterministic outputs from finance impact, evidence retention, capacity planning, data residency, access control, KB quality, runbook coverage, leadership, release, and SLO services, then normalizes them into risk rows, severity, owners, source endpoints, due windows, and acceptance criteria. `POST /risk/register-pack` writes Markdown/JSON under ignored `data/risk_registers/`. It does not create external tickets or call GRC, CRM, billing, HR, Zendesk, Jira, Slack, GitHub, Azure, OpenAI, or external systems.
